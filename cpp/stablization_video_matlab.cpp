    ...
    hVideoSource = vision.VideoFileReader(filename, 'ImageColorSpace', 'Intensity', 'VideoOutputDataType', 'double');
    ...
    BorderCols = [1:pos.search_border(1) + 4W - pos.search_border(1)+4:W];
    BorderRows = [1:pos.search_border(2) + 4H - pos.search_border(2)+4:H];
    ...
    instantiated above to stabilize the input video.
    ...
    input = insertText(input(:,:,1),[191 215],txt,'FontSize',16, 'TextColor', 'white', 'BoxOpacity', 0);
